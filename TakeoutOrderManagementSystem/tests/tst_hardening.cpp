  ...
  void startupRecoveryRejectsInvalidOrUnavailableBackups() {
    {
      ...
      QCOMPARE(context.recoverFromBackup().error().code, ErrorCode::NotFound);
    }
    {
      QTemporaryDir dir;
      QVERIFY(dir.isValid());
      ...
      AppContext context(AppPaths::resolve(dir.path()));
      QCOMPARE(context.initialize().error().code, ErrorCode::CorruptData);
      QCOMPARE(context.recoverFromBackup().error().code, ErrorCode::CorruptData);
    }
    {
      QTemporaryDir dir;
      QVERIFY(dir.isValid());
      ...
      AppContext context(AppPaths::resolve(dir.path()));
      QCOMPARE(context.initialize().error().code, ErrorCode::CorruptData);
      QCOMPARE(context.recoverFromBackup().error().code,
