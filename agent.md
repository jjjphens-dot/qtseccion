...
用户要求依据 Coding Plan **仅实现程序架构**，暂不处理报告，保存重要信息到本文件，并上传至 `https://github.com/jjjphens-dot/qtseccion`。当前 0.2.0 是 AI 辅助架构版本；不是完整 P0，也不能标记为学生纯手写版本。学生手写版、学号 D01、教师对 GitLab/GitHub 提交渠道的要求仍需本人核实。
...
- `core/entities.h` 是值对象模型，使用完整 UUID 字符串、qint64 分、订单快照、可空 claimedAt 等字段；已有完整7状态和支付组合函数，**尚无完整业务状态机与时间/history校验**。
...
| 工作包 | 应继续做的内容 |
|---|---|
| W02 | 补 Validation（账号、价格、字符、容量、引用）、OrderPolicy完整状态/时间/history不变量与测试；保留当前实体和Result契约 |
...
Debug 目录 `build/architecture-debug`，Release 目录 `build/architecture-release`。CTest包含 `architecture`（11个架构用例）与 `shell_smoke`；测试用 QTemporaryDir，不读写正常用户数据。`--smoke-test` 用独立临时目录启动并退出。测试可选环境变量 TAKEOUT_SCREENSHOT 输出窗口截图，仅用于QA。

当前架构测试覆盖 Result、全部21种状态组合、保存失败/过期revision、bootstrap职责分离、已有管理员拒绝初始化、未认证Service拒绝、空库JSON往返与禁止覆盖非空/坏数据、备份保护、QAbstractItemModelTester、数值排序/ID、金额极值、导航不认证与第二实例互斥。

## Git 与发布

开发上传目标 GitHub `jjjphens-dot/qtseccion`；保留原 `origin`（东大GitLab），GitHub使用单独 `github` remote。本轮开始时本地 main 已有课程示例历史，GitHub尚无分支。不得强推、重置或重建历史。
