...
实施记录（2026-09-11）：0.9.0 已完成 W02–W07，并完成 W08 的恢复/导入导出子集。当前具备严格 JSON/原子备份基础、认证凭据机制、管理员首次初始化、用户/骑手注册、商家账号与店铺原子注册、四角色登录注销及真实页面路由；W05 已补齐资料/营业状态、菜品 CRUD、Shop/Dish/Cart Model 与顾客购物车提交/恢复；W06 已补齐订单创建、支付、取消、商家接单/拒单/出餐、骑手认领/送达、顾客确认收货、授权查询和订单页面；W07 已补齐 AdminService、AccountModel、管理员账号查询/逻辑删除、四角色统一统计；W08 已补齐已验证备份的启动恢复确认、管理员数据原子导出/严格导入/上一备份恢复、固定循环密码比较和 hardening 测试。Debug 9 项测试已通过；系统化写入/commit 故障注入、PBKDF2 后台线程和 10,000 单性能测试仍待实施。后续工作包用项目版本号区分，不新增额外内容哈希计算。
...
| P0-Hardening | 自动 `.bak` 检测与恢复交互体验、管理员手动导出/恢复、QLockFile 第二实例处理体验与完整测试、损坏 JSON 测试扩展、写失败/commit 失败错误注入、10,000 单性能测试与优化；沿用现有认证数据格式，不新增额外哈希计算 | Core 门禁通过后在 W08 集中完善；全部 P0 通过后才能进入 P1 |
