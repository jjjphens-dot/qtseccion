...
实施记录（2026-09-13）：0.9.1 已完成 W02–W07，并完成 W08 Hardening 主体。当前具备严格 JSON/原子备份基础、认证凭据机制、管理员首次初始化、用户/骑手注册、商家账号与店铺原子注册、四角色登录注销及真实页面路由；W05 已补齐资料/营业状态、菜品 CRUD、Shop/Dish/Cart Model 与顾客购物车提交/恢复；W06 已补齐订单创建、支付、取消、商家接单/拒单/出餐、骑手认领/送达、顾客确认收货、授权查询和订单页面；W07 已补齐 AdminService、AccountModel、管理员账号查询/逻辑删除、角色统计与管理员平台统计分离；W08 已补齐损坏主文件留存、恢复资格/锁保护、管理员完整备份导入/恢复与 Session 失效、路径别名保护、backup/primary 写入故障注入、固定循环密码比较、PBKDF2 worker、1,200 单混合态和 10,000 单性能样本。Debug/Release 各 9 项测试已通过。后续工作包用项目版本号区分，不新增额外内容哈希计算。
...
| P0-Hardening | 自动 `.bak` 检测与恢复交互体验、管理员手动导出/恢复、QLockFile 第二实例处理体验与完整测试、损坏 JSON 测试扩展、backup/primary 写入失败注入、PBKDF2 worker、混合态与 10,000 单性能测试；沿用现有认证数据格式，不新增额外哈希计算 | W08 已完成；QSaveFile 的 open/write/commit 仍统一通过 AtomicFileWriter 返回错误，测试 seam 覆盖 backup/primary 写入阶段 |
...
默认所有 Model/DataStore 提交在 GUI 线程执行。W04 已完成密码凭据校验；后续工作包沿用现有认证数据格式，不新增额外哈希计算。通过 QObject 父子关系或明确的 `unique_ptr` 管理所有权，避免双重释放；实体使用值语义，不为每种角色强行建立重复继承类。
...
手写版本验证后可标记 `v0.1-handwritten`，完整版本验证后可标记 `v1.0-complete`；名称仅建议，不能据标签推断作者身份。每个版本记录项目版本号、工作包、Kit、构建命令和运行证据，不额外计算内容哈希。保留历史，不通过 squash、强推或重新初始化掩盖开发过程。当前仓库已存在，不重复 `git init`，不照搬 README 中的 force push 命令。
...
| T19 写入失败 | 注入 backup/primary 写入失败；生产 QSaveFile 统一返回 open/write/commit 错误 | 原内存、主文件保持有效；操作返回失败；重启仍为旧版本 |
...
W08 Release 实测（Windows 11、Qt 6.11.2、MinGW 13.1、10,000 个单明细 PendingPayment 订单）：JSON 9,961,704 bytes；encode 218 ms，decode 162 ms，OrderPolicy 10 ms，save 434 ms，load 233 ms，OrderQuery 1 ms，Model projection 4 ms，Statistics <1 ms。该样本用于回归基线，不替代完整课程端到端验收。
...
- [x] P0-Core 门禁先通过；P0-Hardening 的恢复交互、第二实例处理、故障注入、密码实现强化及性能测试全部完成，未被降为选做。
