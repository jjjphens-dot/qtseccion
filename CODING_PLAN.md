...
实施记录（2026-09-12）：0.9.1 已完成 W02–W07，并完成 W08 Hardening 主体。当前具备严格 JSON/原子备份基础、认证凭据机制、管理员首次初始化、用户/骑手注册、商家账号与店铺原子注册、四角色登录注销及真实页面路由；W05 已补齐资料/营业状态、菜品 CRUD、Shop/Dish/Cart Model 与顾客购物车提交/恢复；W06 已补齐订单创建、支付、取消、商家接单/拒单/出餐、骑手认领/送达、顾客确认收货、授权查询和订单页面；W07 已补齐 AdminService、AccountModel、管理员账号查询/逻辑删除、角色统计与管理员平台统计分离；W08 已补齐损坏主文件留存、恢复矩阵、管理员完整备份导入/恢复与 Session 失效、路径别名保护、写入/commit 故障注入、固定循环密码比较和 10,000 单性能样本。Debug/Release 各 9 项测试已通过；PBKDF2 后台线程优化仍需独立 UI 异步改造。后续工作包用项目版本号区分，不新增额外内容哈希计算。
...
| P0-Hardening | 自动 `.bak` 检测与恢复交互体验、管理员手动导出/恢复、QLockFile 第二实例处理体验与完整测试、损坏 JSON 测试扩展、写失败/commit 失败错误注入、10,000 单性能测试与优化；沿用现有认证数据格式，不新增额外哈希计算 | W08 主体已完成；PBKDF2 后台线程作为后续 UI 异步优化，不改变当前同步认证正确性 |
...
| W08 | 9/16–9/18，8–11h | W07 Core 门禁 | 完善备份恢复体验及管理员导出/恢复、第二实例处理、损坏 JSON 扩展、写失败/commit 错误注入、10,000 单性能测试；沿用现有认证数据格式，不新增额外哈希计算；执行完整 P0 功能回归 | T17–T22 强化子项、全部 P0 功能测试及本机 Release 回归通过；Core 不退化；9/18 冻结功能并保留第二版可运行提交；目标机器最终发行验收在 W10 |
