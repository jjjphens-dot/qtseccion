...
实施记录（2026-09-07）：0.7.0 已完成 W02–W06。当前具备严格 JSON/原子备份基础、已完成的认证凭据机制、管理员首次初始化、用户/骑手注册、商家账号与店铺原子注册、四角色登录注销及真实页面路由；W05 已补齐资料/营业状态、菜品 CRUD、Shop/Dish/Cart Model 与顾客购物车提交/恢复；W06 已补齐订单创建、支付、取消、商家接单/拒单/出餐、骑手认领/送达、顾客确认收货、授权查询和订单页面。规则、持久化、认证、目录购物车、订单闭环和 UI 契约测试通过。管理统计与恢复 UI 按 W07–W08 实施。后续工作包用项目版本号区分，不新增额外内容哈希计算。
